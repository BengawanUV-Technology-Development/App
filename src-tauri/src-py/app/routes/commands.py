...
from app.utils.errors import ErrorCode, InvalidRequestError, NotConnectedError
from app.services.command import CommandService

command_bp = Blueprint("command", __name__, url_prefix="/command")
_command_service: CommandService | None = None

def init_command_routes(state_manager):
    global _command_service
    _command_service = CommandService(state_manager)
...
def takeoff_command():
    ...
    try:
        result = service.execute_takeoff(data.get("altitude_m"))
        return jsonify(result), 200
    except NotConnectedError:
...
def set_takeoff_altitude_command():
    ...
    try:
        result = service.execute_set_takeoff_altitude(data.get("altitude_m"))
        return jsonify(result), 200
    except NotConnectedError:
...
def _error_response(command_name: str, error_code: ErrorCode, error_message: str):
