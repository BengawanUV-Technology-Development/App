...
class VisionDetectionService:
    ...
    def record_telemetry(self, sample: TelemetrySample | Mapping[str, Any]) -> bool:
        ...
        with self._lock:
            ...
            if sample_mission_id not in (None, self._mission_id):
                return False
            return self._timeline.append(sample)
    ...
    def ingest_event(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        with self._lock:
            try:
                ...
                # A live estimate is never a qualification claim.  A ground
                # truth target and validated calibration are still required.
...
def _normalized_bbox(value: Any) -> list[float]:
    ...
    if (
        ...
        raise VisionIngestError(
            "bbox coordinates must be normalized XYXY values in [0, 1]",
            code="BBOX_INVALID",
        )
    return bbox
