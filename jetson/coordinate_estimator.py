...
src-tauri/src-py/app/services/coordinate_estimator.py, not an import of
it: the jetson/ scripts run in a separate Python environment on the
Jetson (no Flask/app package available there), and this module is used
offline by generate_detection_snapshots.py against the Jetson's own
onboard telemetry.jsonl -- see docs/post-flight-detection-snapshots.md
for why that sidesteps the ground telemetry clock-sync question
entirely. Keep the two files' math in sync if you change one.
