            ...
            while True:
                try:
                    ...
                    with self._lock:
                        ...
                        self.stats[kind].setdefault("retries", 0)
                        self.stats[kind]["retries"] += 1
                    if now - self._last_error_at >= 5:
                        print(f"[detector] {utc_iso(time.time())} {kind} ingest unavailable; retrying: {exc}", flush=True)
                        self._last_error_at = now
                    # Event records are retried in order while the recording
                    # continues. Overlay remains newest-only and is allowed
                    # to stop promptly during shutdown.
                    if kind != "event" or self._stop_event.wait(retry_delay):
                        break
...
class DetectionWorker:
    ...
    def _initialize_detector(self) -> None:
        """Load and verify YOLO without delaying camera capture startup."""

        try:
            manifest = ProductionModelManifest(self.args.model_manifest)
            ...
            verified_model = manifest.verify(self.args.weights)
            runner = DetectionRunner(self.args, verified_model)
            with self._lock:
            ...
            print("[detector] YOLO model initialized", flush=True)
        except Exception as exc:
            with self._lock:
                self.status = "FAILED"
                self.failure_reason = f"DETECTOR_INITIALIZATION_FAILED: {exc}"
            print(f"[detector] initialization failed; capture continues: {exc}", flush=True)
    ...
    def start(self) -> None:
        ...
        self.initialization_thread = threading.Thread(
            target=self._initialize_detector,
            daemon=True,
            name="jetson-yolo-initializer",
        )
    ...
    def close(self) -> None:
        ...
        if self.initialization_thread:
            self.initialization_thread.join(timeout=2)
